...
(
 ...
 uniform highp float shiftX;
 uniform highp float shiftY;
 
 void main()
 {
     lowp vec4 sharpImageColor = texture2D(inputImageTexture, textureCoordinate);
     lowp vec4 blurredImageColor = texture2D(inputImageTexture2, textureCoordinate2);
     highp vec2 textureCoordinateToUse = vec2(textureCoordinate2.x + 0.5 - shiftX, (textureCoordinate2.y + 0.5 - shiftY));
...
@synthesize blurPoint = _blurPoint;
...
- (id)init;
{
    ...
    self.blurPoint = CGPointMake(0.5, 0.5);
    return self;
}
...
- (void)setBlurPoint:(CGPoint)blurPoint
{
    _blurPoint = blurPoint;
    [selectiveFocusFilter setFloat:blurPoint.x forUniformName:@"shiftX"];
    [selectiveFocusFilter setFloat:blurPoint.y forUniformName:@"shiftY"];
}
@end
